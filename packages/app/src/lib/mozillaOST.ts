...
The CLI itself feels foreign to non-developers across PM and design. Consistently named across multiple interviews as the primary barrier to getting started.

##### [Solution] Claude Code Desktop rollout

In progress — design team is first on the list. Approval and budget secured. Directly addresses terminal barrier.

#### [Opportunity] Tool confusion

Cursor, VS Code, Claude Code, terminal — what's approved, how do they relate, which to use? The approved tool landscape is genuinely confusing and navigating it has required trial and error.
...
Low-stakes, hands-on sessions where people build workflows for their own pain points. Modeled on the design team's existing office hours.

#### [Solution] AI for PM Slack channel

Async version of office hours — a dedicated channel for sharing what's working.

#### [Solution] Peer learning groups

The only known grassroots PM peer learning community works because it's low-stakes and psychologically safe. Scale the peer-safety model across the PM org.

#### [Solution] Firefox PM OS starter repo

A starter plugin/GitHub repo designed for Firefox PM work. Easy to activate, clear path to customize, everyone contributes back to the source.

#### [Solution] Hands-on product-wide session

A 2-hour hands-on session (design org ran one that was highly effective). Creates shared vocabulary and baseline. Pair with tool setup rather than starting in the deep end.
...
AI surfaces patterns across recent 1:1 notes, flags blockers, drafts talking points. Strong positive signal from managers who've seen a demo.

##### [Experiment] Mozilla policy allows AI folders referencing direct reports

Unconfirmed whether policy prohibits keeping AI project folders referencing direct reports. Must resolve before piloting.
...
"The path to go from idea to reality is very unclear." AI could help with visibility, dependency mapping, or stakeholder tracking.
...
Shared prompt library, templates, and workflows for common PM tasks. Consistently named as a top need — everyone is rebuilding the same context from scratch.

#### [Solution] "What's worth scaling" decision owner

Monthly show-and-tell surfaces good work but nothing moves it forward. Someone needs to own the "what's worth scaling" call.

#### [Solution] Increase sharing cadence

Once/month might as well not exist. Weekly or biweekly async Slack sharing keeps pace with how fast tools are changing.
...
A design component library is already built but blocked by SSO — sitting there undeployed.
...
PMs without team-level TPMs spend significant time on routine status updates — estimated at 1.5+ hrs/week. Formulaic work ripe for automation.

#### [Solution] TPM tool pattern → PM status-update communicator

A TPM in the org has already built a complete tool (Jira pulls, weekly reports, Slack automation, staging). A version scoped to PM status updates could be high-leverage.

##### [Experiment] The TPM tool pattern generalizes to PM workflows

Built for a specific TPM workflow. Needs validation that the core pattern transfers to PM status reporting without significant rebuilding.
...
Content design could co-develop this. Leadership has explicitly named brief redefinition as a priority.
...
The CLG has AI expectations that haven't been communicated yet. Anxiety from an invisible bar is worse than a challenging visible one.

#### [Solution] Explicit "collaborative not competitive" reframe from leadership

The tone is set at the top. If leadership frames AI adoption as team capability, not individual performance, it changes the context for everything else.
...
Setting up a local Firefox copy is a significant time investment — requiring a full day that can't happen during normal business hours. A precondition for the most powerful prototyping workflows.
...
Single source so far. Needs validation — may be a niche constraint rather than a widespread blocker.
`;
